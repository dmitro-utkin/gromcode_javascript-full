...
// Updated renderTasks function with the modified sort logic
const renderTasks = tasksList => {
  listElem.innerHTML = '';
  
  // Separating completed and active tasks
  const completedTasks = tasksList.filter(task => task.done);
  const activeTasks = tasksList.filter(task => !task.done);
 
  // Sorting the completed tasks by the most recently completed first
  completedTasks.sort((a, b) => {
    return new Date(b.doneDate) - new Date(a.doneDate);
  });
 
  // Sorting the active tasks by the most recently added first
  activeTasks.sort((a, b) => {
    return new Date(b.addedDate) - new Date(a.addedDate);
  });
 
  // Concatenating the sorted active and completed tasks
  const sortedTasks = activeTasks.concat(completedTasks);
 
  // Creating and appending the task elements
